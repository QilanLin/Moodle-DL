...
class TestTocHrefRewritingContract:
    ...
    def test_toc_href_should_use_on_disk_folder_name_with_prefix(self):
        ...
        *NN* prefix and the chapter title. The TOC href must
        ...
        # After fix, the chapter's on-disk folder name is
        # generated by _format_chapter_folder_name with the
        # *NN* prefix included.
        #
        # Per book.py:486-489:
        #   if chapter_number:
        #     return PT.to_valid_name(f'{chapter_number} {chapter_title}',
        #                             is_file=False)
        #   return PT.to_valid_name(f'{fallback_index:02d} - {chapter_title}',
        #                          is_file=False)
        # The fix should ALSO prepend the *NN* prefix (e.g. *07*).
        expected_folder_with_prefix = '*07* ' + chapter_title
        ...
        toc_html = BookMod.create_ordered_index(items=toc)
...
class TestPrintBookVideoSrcContract:
    ...
    def test_format_chapter_folder_name_after_fix_includes_prefix(self):
        ...
        This test is RED on the current code and will be GREEN
        after the fix.
        """
        from moodle_dl.moodle.mods.book import BookMod

        # Pin the EXPECTED behavior: when called with chapter_number
        # '1.1' and fallback_index 0, the result should be a folder
        # name that matches the ON-DISK folder (with *NN* prefix).
        # The exact format depends on the fix design; the simplest
        # expected format is '*01* 1.1 Week Overview' (or similar).
        result = BookMod._format_chapter_folder_name(
            chapter_title='Week Overview',
            chapter_number='1.1',
            fallback_index=0,
        ...
        # The result should start with *NN* (the on-disk prefix).
        # We allow multiple valid formats:
        #   '*01* 1.1 Week Overview'
        #   '*1.1* Week Overview' (less common)
        # We just require that the result CONTAINS a *NN* prefix
        # pattern.
        import re as _re
        assert _re.match(r'\*\d{2}\*\s', result) or _re.search(r'\*\d', result), (
            f'After fix, _format_chapter_folder_name should include '
            f'a *NN* prefix in its output. Got: {result!r}. '
            f'The prefix is added by gen_path later, but book.py:303 '
            f'passes this value to _create_linked_print_book_html, '
            f'where the *NN* prefix is LOST (it only exists in the '
            f'on-disk path, not in the field passed to the print book).'
        )
