...
@pytest.mark.asyncio
async def test_kalvidres_halt_videos_writes_notes_and_skips_video(task_factory, tmp_path):
    """When opts.halt_videos is True, _handle_kalvidres_download writes
    _notes.md (annotation / description / transcript) but skips the
    video download. The DB row is marked [HALTED] so --release-videos
    can pick it up later."""
    opts = MoodleDlOpts()
    opts.halt_videos = True
    options = DownloadOptions(
        token='token-abc',
        moodle_url='https://moodle.example.com',
        download_linked_files=False,
    ...
    file = MagicMock()
    file.content_filepath = '/'
    file.content_filename = 'Week 01 - Overview.mp4'
    file.content_fileurl = 'https://keats.kcl.ac.uk/mod/kalvidres/view.php?id=9154816'
    file.content_filesize = 0
    file.content_timemodified = 123
    file.content_type = 'cookie_mod'
    ...
    task = Task(1, file, course, options, pool, callback)
    task.file.saved_to = str(tmp_path / 'Week 01 - Overview.mp4')
    task.extract_kalvidres_text = AsyncMock()
    task.extract_kalvidres_video_url = AsyncMock()
    task.external_download_url = AsyncMock()
    task._build_kaltura_url_from_known_embed_url = MagicMock(return_value=None)

    # Inject a fake DB that records the [HALTED] save_failed_file call.
    fake_db = MagicMock()
    fake_db.save_failed_file = MagicMock()
    task.database = fake_db

    await task._handle_kalvidres_download()

    # Annotation extraction was attempted.
    task.extract_kalvidres_text.assert_awaited_once_with(
        'https://keats.kcl.ac.uk/mod/kalvidres/view.php?id=9154816',
        str(tmp_path / 'Week 01 - Overview_notes.md'),
    )
    # Video URL extraction and download were NOT called.
    task.extract_kalvidres_video_url.assert_not_awaited()
    task.external_download_url.assert_not_awaited()
    # DB was marked [HALTED].
    fake_db.save_failed_file.assert_called_once()
    reason_arg = fake_db.save_failed_file.call_args.kwargs['error_message']
    assert reason_arg.startswith('[HALTED]')
    # Task reported success so download_service progress counter increments.
    statuses = [e for e in events if e[0] == 'finished' or e[0].name == 'FINISHED']
    assert statuses, f'expected FINISHED event, got events: {events}'
