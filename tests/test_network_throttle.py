...
def test_network_throttle_waits_before_first_and_queues_following_requests():
    ...
    # First wait: delay=3.0+1.0*0.0=3.0, accumulated 3.0
    # Second wait: delay=3.0+1.0*0.0=3.0, accumulated 6.0 (queued behind first)
    assert [call.args[0] for call in sleep.call_args_list] == [3.0, 6.0]
...
async def test_network_throttle_async_wait_uses_async_sleep():
    ...
    # delay = base_delay (3.0) + jitter (1.0) * random (1.0) = 4.0
    async_sleep.assert_awaited_once_with(4.0)
...
async def test_network_throttle_async_wait_does_not_prebook_concurrent_callers():
    ...
    for expected_count in (2, 3):
        ...
        await asyncio.sleep(0)
        await asyncio.sleep(0)
        assert sleep_delays == [3.0] * expected_count

    waiter, _delay = sleep_waiters.pop(0)
    waiter.set_result(None)
    await asyncio.gather(*tasks)

    assert sleep_delays == [3.0, 3.0, 3.0]