...
class ForumMod(MoodleMod):
    ...
    MOD_MIN_VERSION = 2019052000  # 3.7 - 论坛讨论功能需要 Moodle 3.7+
    ...
    async def _fetch_discussions_mobile_api(
        ...
        # mod_forum_get_forum_discussions_paginated 已在 Moodle 3.11 弃用，4.4 移除
        # 最低支持版本: Moodle 3.7 (2019052000)
        if self.version < 2019052000:  # < 3.7
            raise NotImplementedError(
                '论坛讨论功能需要 Moodle 3.7 或更高版本。\n'
                'mod_forum_get_forum_discussions_paginated API 已在 Moodle 3.11 弃用，4.4 移除。'
            )
    ...
    async def _fetch_discussions_web_api(self, forum: Dict, last_timestamp: int) -> List[Dict]:
        ...
        logging.warning(
            ...
            f'\n   2. 确保 Moodle 版本 >= 3.7 (支持新版 forum discussions API)'
            f'\n   3. 检查用户是否有查看讨论的权限'
        )
    ...
    async def _fetch_discussion_posts_mobile_api(self, data: Dict) -> List[Dict]:
        """
        使用 Mobile API (mod_forum_get_discussion_posts) 获取讨论帖子。
        ...
        if self.version < 2019052000:  # < 3.7
            raise NotImplementedError(
                '论坛帖子功能需要 Moodle 3.7 或更高版本。\n'
                'mod_forum_get_forum_discussion_posts API 已被 mod_forum_get_discussion_posts 替代。'
            )
