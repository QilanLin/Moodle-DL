...
class PageMod(MoodleMod):
    ...
    @classmethod
    def _page_contains_kaltura_video(cls, page_content: str, page_intro: str) -> bool:
    ...
    @classmethod
    def _is_kaltura_helper_icon_from_page_content(cls, file: Dict) -> bool:
    ...
    @classmethod
    def _is_small_kaltura_helper_icon(cls, file: Dict) -> bool:
        if not cls._is_kaltura_helper_icon_from_page_content(file):
            return False
    ...
    @classmethod
    def _filter_adjacent_kaltura_helper_icons(
        cls, page_files: List[Dict], page_content: str, page_intro: str
    ) -> List[Dict]:
        if not cls._page_contains_kaltura_video(page_content, page_intro):
            return page_files
        ...
        for index in range(0, len(filenames) - run_length + 1):
            ...
            if (
                ...
                and all(cls._is_kaltura_helper_icon_from_page_content(file) for file in run_files)
            ):
        ...
        return [
            ...
            if index not in icon_run_indexes or not cls._is_small_kaltura_helper_icon(file)
        ]
    ...
    async def real_fetch_mod_entries(
        ...
        for page in pages:
            ...
            page_files = self._filter_adjacent_kaltura_helper_icons(page_files, page_content, page_intro)
